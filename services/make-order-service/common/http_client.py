...
def request_with_cb(
    ...
    **kwargs: Any,
    ...
    # Nếu circuit đang OPEN, kiểm tra xem đã đủ thời gian reset chưa
    if current_state == "OPEN":
        # PyBreaker tự động chuyển sang HALF-OPEN sau reset_timeout
        # Nếu vẫn OPEN, chặn request
        if breaker.current_state == "open":
            raise HTTPException(
                status_code=503,
                detail=f"Circuit OPEN for {service_name}: Error rate exceeded {CB_ERROR_RATE_THRESHOLD:.0%}"
            )
    ...
    if _check_error_rate(service_name):
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Circuit OPEN for {service_name}: Error rate {_get_current_error_rate(service_name):.0%} exceeded threshold {CB_ERROR_RATE_THRESHOLD:.0%}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"{service_name} unreachable: {exc}"
        )
